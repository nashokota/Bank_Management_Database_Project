...
@section('title', 'Edit Loan - Banking Management System')

@section('content')
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><i class="fas fa-edit me-2"></i>Edit Loan #{{ $loan->id }}</h1>
    <div class="btn-toolbar mb-2 mb-md-0">
        <div class="btn-group">
            <a href="{{ route('loans.show', $loan) }}" class="btn btn-info">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('loan_type') is-invalid @enderror" 
                                    ...
                                    <option value="home" {{ (old('loan_type') ?? $loan->loan_type) == 'home' ? 'selected' : '' }}>Home Loan</option>
                                    <option value="car" {{ (old('loan_type') ?? $loan->loan_type) == 'car' ? 'selected' : '' }}>Car Loan</option>
                                    <option value="education" {{ (old('loan_type') ?? $loan->loan_type) == 'education' ? 'selected' : '' }}>Education Loan</option>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('start_date') is-invalid @enderror" 
                                       ...
                                       value="{{ old('start_date') ?? ($loan->start_date ? $loan->start_date->format('Y-m-d') : '') }}" required>
                                @error('start_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control @error('end_date') is-invalid @enderror" 
                                       ...
                                       value="{{ old('end_date') ?? ($loan->end_date ? $loan->end_date->format('Y-m-d') : '') }}" required>
                                @error('end_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
